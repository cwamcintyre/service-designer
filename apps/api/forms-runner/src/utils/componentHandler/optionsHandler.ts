...
export class OptionsComponentHandler implements ComponentHandler {
    ...
    Convert(component: Component, data: { [key: string]: any }): any {
        if (component.name) {
            ...
            const matchingOption = component.options?.find(option => option.id == selectedValue?.id);
            ...
            if (!matchingOption && !component.optional && data[component.name]) {
                throw new Error(`No matching option found or invalid option type for value: ${selectedValue?.value} in component: ${component.name}`);
            }
